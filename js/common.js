...
function addToCart(goodsId,parentId)
{
  ...
  var img=obj.parents("dl").find("img");
  goods.url      = img.attr("src");
  goods.left      = left;
  goods.top      = top;
  goods.quick    = quick;
  goods.spec     = spec_arr;
...
function addToCartResponse(result)
{
  ...
  else
  {
    ...
    else
    {
	  ...
	  createFly(result.url,result.left,result.top);
	  
    }
  }
}

function createFly(url,l,t){
	var img=$("<img class='fly' />");
	l = parseInt(l);
	t = parseInt(t);
	var t2=$(".hd_cart").offset().top;
	var l2=$(".hd_cart").offset().left
	img.attr("src",url).css({"left":l,"top":t}).animate({top:t-100},function(){
		$(this).animate({top:t2,left:l2},"slow",function(){
			$(this).remove();
			tips_out(l2);
		});
	});
...
function tips_out(left){
	var div=$(".cart_result").clone(true);
	div.css("left",left-55).animate({top:10,opacity:1},"slow",function(){
		div.timer=setTimeout(function(){
			div.animate({top:-10,opacity:0},function(){
				div.remove();
			});
		},2000);
	});
