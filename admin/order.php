...
foreach ($data as $key => $value) {
    foreach ($value as $k => $v) {
       $name = iconv('utf-8','gb2312',$v['consignee']); //客户姓名 
        $sex = iconv('utf-8','gb2312',$v['address']); 
       

        // $str .= "C,,7100334,S1,,".$name.",,".$sex.",,,,".$v['city'].",".$v['email'].",,,".$v['zipcode'].",AU,".$v['mobile'].",Y,SIGNATURE IS REQUIRED,".$v['order_sn'].",Y,".$V['email'].",TRACKADV\n"; //用引文逗号分开 
        $str .="C,b,7100334,S1,e,".$name.",g,".$sex.",i,j,k,".$v['city'].",m,".$v['zipcode'].",AU,".$v['mobile'].",Y,r,SIGNATURE IS REQUIRED,Y,u,v,w,x,".$v['email'].",TRACKADV\n";
        $str .="A,0.5,5,5,5,1\n";
    }
}
...
function create_item($consignee,$address,$city,$zipcode,$email,$mobile,$order_sn)
{

    $item = "<order>\n";
    $item .= "<companyName>$consignee</companyName>\n";
    $item .= "<address1>$address</address1>\n";
    $item .= "<address2></address2>\n";
    $item .= "<suburb>$city</suburb>\n";
    $item .="<postcode>$zipcode</postcode>\n";
    $item .="<emailAddress>$email</emailAddress>\n";
    $item .="<phone>$mobile</phone>\n";
    $item .="<specialInstruction1>SIGNATURE MUST BE REQUIRED</specialInstruction1>\n";
    $item .="<items>\n";
    $item .="<item>\n";
    $item .="<reference>$order_sn</reference>\n";
    $item .="<packaging>7</packaging>\n";
