...
if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'jishuanqi'){


$code = $_POST['code1'];
$sql = "SELECT `SubName` FROM ecs_huntermain WHERE PostCode = $code";
$bought_notes = $db->getAll($sql);
 echo json_encode($bought_notes);
die;
}

if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'jishuan'){
$code = $_POST['code1'];
// $code = "ABBOTSFORD";
$sql = "SELECT `ZoneName` FROM ecs_huntermain WHERE SubName = '$code'  LIMIT 1";
$bought_notes = $db->getAll($sql);
$zone =$bought_notes[0]['ZoneName'];
//拿到subname去查zouname// 
$sql = "SELECT * FROM ecs_hunterzone WHERE ZoneName = '$zone'";
$data = $db->getAll($sql);
...
// if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'zuizhong'){
...
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_weight,g.goods_img, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'g.market_price, g.promote_price, g.promote_start_date,g.other_long,g.other_wide,g.other_high, g.promote_end_date ' .
